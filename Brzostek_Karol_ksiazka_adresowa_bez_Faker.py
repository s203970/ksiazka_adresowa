...
tk.Button(root, text="Wyświetl wszystkie wpisy.", command=wczytaj_dane).grid(row=6, columnspan=2)

# Wyszukiwanie wpisów.
tk.Label(root, text="Wybierz, po jakiej wartości chcesz przeszukać dane:").grid(row=7, column=0, sticky="w")
...
opcje_menu.grid(row=7, column=1, sticky="w")

tk.Label(root, text="Wpisz wartość, jaka ma zostać wyszukana: ").grid(row=8, column=0, sticky="w")
entry_szukaj=tk.Entry(root)
entry_szukaj.grid(row=8, column=1, sticky="w")

tk.Button(root, text="Wyszukaj", command=wyszukaj_dane).grid(row=7, rowspan=2, column=1)
...
table.grid(row=9, columnspan=2)

tk.Button(root, text="Statystyki miast", command=pokaz_statystyki).grid(row=10, column=0, sticky="nw")

wczytaj_dane()


tk.Button(root, text="Usuń zaznaczony wpis.", command=usun_wpis).grid(row=11, columnspan=2)

tk.Button(root, text="Wyczyść wszystkie dane z pliku", command=wyczysc_dane).grid(row=12, columnspan=2)

tk.Button(root, text="Zamknij program.", background="#fc4c4c", command=zamknij_program).grid(row=13, columnspan=2)
